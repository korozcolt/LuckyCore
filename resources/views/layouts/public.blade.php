    ...
    <header class="sticky top-0 z-50 bg-white dark:bg-[#102210] border-b border-[#dbe6db] dark:border-white/10 px-4 md:px-10 py-3">
        <div class="max-w-[1280px] mx-auto flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 lg:gap-8">
                ...
                {{-- Cart Button --}}
                <a href="{{ route('cart.index') }}" class="relative p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" wire:navigate>
                    <span class="material-symbols-outlined text-[24px]">shopping_cart</span>
                    @if(($cartCount ?? 0) > 0)
                        <span class="absolute top-1 right-1 bg-[#13ec13] text-[10px] font-bold px-1.5 py-0.5 rounded-full text-black">{{ $cartCount }}</span>
                    @endif
                </a>
